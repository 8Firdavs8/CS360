...
class SquareCommand:
    def __init__(self, radius=1, width=1, color="orange"):
        self.radius = radius
        self.width = width
        self.color = color
    ...
    def __str__(self):
        return '<Command width="' + str(self.width) + '">Square</Command>'
...
class DrawingApplication(tkinter.Frame):
    ...
    def buildWindow(self):
        ...
        # This is a label widget. Packing it puts it at the top of the sidebar.
        sLabel = tkinter.Label(sideBar,text="Square Input")
        sLabel.pack()
        sSize = tkinter.StringVar()
        sEntry = tkinter.Entry(sideBar,textvariable=sSize)
        sEntry.pack()
        sSize.set(str(15))
        ...
        def sHandler():
            cmd = SquareCommand( float(radiusSize.get()), float(sSize.get()), penColor.get())
            cmd.draw(theTurtle)
            self.graphicsCommands.append(cmd)
            screen.update()
            screen.listen()
        ...
        sButton = tkinter.Button(sideBar, text = "Draw Square", command=sHandler)
        circleButton.pack(fill=tkinter.BOTH)  
        sButton.pack(fill=tkinter.BOTH)       
